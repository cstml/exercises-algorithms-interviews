...
bool insertNote (std::map<std::string, int> &h, string &word){
    if (h.find(word) != h.end()){
        h[word] = h[word] + 1;
        return true;
    }
    else{
        h.insert(std::pair<string,int>(word, 1));
        return true;
    }
}

bool eraseNote (std::map<std::string, int> &h, string &word){
    if (h.find(word) == h.end()){
        return false;
    }
    else{
