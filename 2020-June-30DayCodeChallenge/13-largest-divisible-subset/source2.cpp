...
class Solution {
public:
    int ColumnMax(unsigned int i, std::vector<std::vector<int>> vec)
    {
        int maxT=0;
    ...
    void PrintM (std::vector<std::vector<int>>& mat, std::vector<int>& nums)
    {
        std::cout<<"  ";
        for (int a : nums)
            std::cout<<a<<" ";
    ...
    std::vector<int> largestDivisibleSubset(std::vector<int>& nums) 
    {
        ...
        RevTrail (mat,maxI,maxJ,ans,nums);
    ...
    void RevTrail ( std::vector<std::vector<int>> mat, unsigned int maxI, unsigned int maxJ, std::vector<int>& ans, std::vector<int>&vec)
    {
        ans.push_back(vec[maxI]);
        while (mat[maxI][maxJ])
        {
            std::vector<int>::iterator it = ans.begin();
            int nMax=0;
            int nI=0;
            while (maxI)
            {
                maxI--;
                if (nMax<mat[maxI][maxJ])
                {
                    nMax=mat[maxI][maxJ];
                    nI=maxI;
                }
